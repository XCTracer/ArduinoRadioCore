/* 
 * Simple GFSK, with Manchester decoding, TX and RX, synchword and preamble from ADS-L V2
 *    
...
void manchester_decode(uint8_t* dest_buffer, uint8_t* source_buffer, size_t dest_length) {
    for (size_t i = 0; i < dest_length; i++) {
        uint8_t d_byte = 0;

        uint8_t m_byte = *source_buffer++;
        if (m_byte & 0x40) { d_byte |= 0x80; }
        if (m_byte & 0x10) { d_byte |= 0x40; }
        if (m_byte & 0x04) { d_byte |= 0x20; }
        if (m_byte & 0x01) { d_byte |= 0x10; }

        m_byte = *source_buffer++;
        if (m_byte & 0x40) { d_byte |= 0x08; }
        if (m_byte & 0x10) { d_byte |= 0x04; }
        if (m_byte & 0x04) { d_byte |= 0x02; }
        if (m_byte & 0x01) { d_byte |= 0x01; }
...
void setup(void) {
    ...
    Serial.println("GFSK, Manchester decoding, RX");
...
void loop(void) {
    ...
    if (radio.has_packet()) {
        ...
        radio.read_packet(message, sizeof(message), info);
        manchester_decode((uint8_t *)decoded, message, sizeof(decoded));
